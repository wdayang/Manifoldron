...
def point_to_envelop_kd(data_1,dictionary1,point):
    #smallest1 = 999999
    dict_all = np.unique(np.reshape(dictionary1,(dictionary1.shape[0]*dictionary1.shape[1])))
    points = data_1[dict_all]
    
    tree = KDTree(points, leaf_size=40)              
    dist, ind = tree.query(point.reshape(1,-1), k=2) 
...
def distance_kd(X, n_neighbors=14):
    length = X.shape[0]
    n_neighbor = np.min([length,n_neighbors+1])
    neighbors = np.zeros((X.shape[0],X.shape[0]))
    tree = KDTree(X, leaf_size=40)              
    dist, ind = tree.query(X, k=n_neighbor) 
    ind = ind[:,1:]
    for k,i in enumerate(ind):
        neighbors[k,i] = 1  ## make sure it's not 0
    neighbors = (neighbors+neighbors.T)/2
    
    return neighbors,dist

def distance_kd2(X, n_neighbors=14):
    length = X.shape[0]
    n_neighbor = np.min([length,n_neighbors+1])
    neighbors = np.zeros((X.shape[0],X.shape[0]))
    tree = KDTree(X, leaf_size=40)              
