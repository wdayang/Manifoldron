...
x = pd.read_csv('data/Letter-Recognition-Data-Set-master/letter-recognition.data',header=None)
...
x_class = x.iloc[:,:1]
x_train = x.iloc[:,1:]
x_class = np.array(x_class,dtype=np.float)
x_train = np.array(x_train,dtype=np.float)
X,X_test,y,y_test,= train_test_split(x_train, x_class,test_size=0.2,random_state=30)
...
def one_round(indexes):
    ...
    for cl in range(n_class):
        ...
        d_1,_ = np.where(y==cl) ## the id of the points, 0,1,2,3.... Sometimes use '_' sometimes not. 
        data_1 = X[d_1]
        #print(data_1.shape)
