...
data_ori = pd.read_csv('data/drug_consumption.data')
data = data_ori.apply(le.fit_transform)  ## categorical to float
data = data.to_numpy()
#%% specific dealing
X_all = data[:,:-1]
y_all = data[:,-1]
index1 = np.where(y_all != 6)
X_all = X_all[index1]
y_all = y_all[index1]
#X_all = np.delete(X_all,2,1)  ## delete dependant columns 2
...
feature_use = 6
n_bag = (num_features-feature_use)*2  # num_features > feature_use
...
for bag in range(n_bag):
    ...
    feature_use = np.random.randint(3,7)  ## features = 3,4,5,6
