...
training_set = pd.read_csv('data/sat/sat.trn', delimiter=' ', nrows = None, header = None)
...
testing_set = pd.read_csv('data/sat/sat.tst', delimiter=' ', nrows = None, header = None)
...
X = X + np.random.normal(0,0.3,X.shape)  ## add some disturbance to data
...
def one_round(indexes):
    ...
    dist_all2 = np.ones((y_test.shape[0],n_class),dtype=np.float)*-1
    ...
    for cl in range(n_class):
        ...
        neighbors1, distances,tree = distance_kd2(data_1,n_neighbor)
        ...
        dist, ind = tree.query(X_test, k=4)     ## get the closest
        ind = ind[:,1]  ## only need to get the closest for in-out simplex detection
        dist = dist[:,1:]
        dist_all2[:,cl] = np.mean(dist,axis=1)
        
# =============================================================================
#         # go over all nearest points to test in-out simplex
#         for i in range(ind.shape[0]):
#             for j in range(tri1_new.shape[0]):
#                 if ind[i] in tri1_new[j]:  ## find cooresponding simplex
#                     hulls = data_1[tri1_new[j]]
#                     inhull_detect = in_hull_batch(hulls, X_test[i].reshape(1,-1))
#                     pred_all[i,cl] = 1 * inhull_detect
# =============================================================================
        #%% in-out simplex detection
...
#                 dist_min[i,ind] = point_to_envelop_kd(data_1,dictionary1,X_test[i])
#             pred[i] = np.amin(np.where(dist_min[i] == np.amin(dist_min[i])))
# =============================================================================
            pred[i] = np.amin(np.where(dist_all2[i] == np.amin(dist_all2[i])))
            outhull = outhull + 1
...
#             for ind,(data_1,dictionary1) in enumerate(zip(data_all,dictionary_all)):
#                 if pred_all[i,ind] == 1:
#                     dist_max[i,ind] = point_to_envelop_kd(data_1,dictionary1,X_test[i])
# =============================================================================
            pred[i] = np.amin(np.where(dist_all2[i] == np.amax(dist_all2[i])))
