...
#%% load dataset
data_ori = pd.read_csv('data/parkinsons.data')
data = data_ori.to_numpy()

X_all = np.hstack((data[:,1:17],data[:,18:]))
y_all = data[:,17]
...
def one_round(indexes):
    ...
    one_round.counter += 1
    print("round %d/%d acc: %f, time %.2f"%(one_round.counter,n_bag//4+1,correct,time.time()-time_st2))
    #return pred
